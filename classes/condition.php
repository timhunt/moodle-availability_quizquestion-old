...
/**
 * Condition on the state of a particular question in a particular quiz for the user.
 *
...
class condition extends \core_availability\condition {
    /** @var string used to indicate the condition is on not having an attempt. */
    const ATTEMPT_NONE = 'notattempted';

    /** @var string used to indicate the condition is on the most recent attempt. */
    const ATTEMPT_MOST_RECENT = 'mostrecent';
    ...
    /** @var string Which of the user's attempts to consider. One of the ATTEMPT_ consts above */
    private $whichattempt;
    ...
    public function __construct($structure) {
        ...
        if (isset($structure->id) && is_int($structure->id)) {
            $this->quizid = $structure->id;
        } else {
            throw new \coding_exception('Missing or invalid quiz ->id for quizquestion condition');
        }

        // Get the quiz id.
        if (!isset($structure->attempt)) {
            throw new \coding_exception('Missing ->attempt for quizquestion condition');
        } else if ($structure->attempt === self::ATTEMPT_NONE) {
            $this->whichattempt = self::ATTEMPT_NONE;
    ...
    public function is_available($not, \core_availability\info $info, $grabthelot, $userid) {
        ...
        $score = $this->get_question_state($this->gradeitemid, $course->id, $grabthelot, $userid);
        $allow = $score !== false &&
    ...
    public function get_description($full, $not, \core_availability\info $info) {
        ...
        return get_string('requires_' . $string, 'availability_quizquestion', $name);
    }

    protected function get_debug_string() {
        $out = '#' . $this->quizid . ' ' . $this->whichattempt;
        if (!is_null($this->min)) {
    ...
    private static function get_cached_grade_name($courseid, $gradeitemid) {
        ...
        $cache = \cache::make('availability_quizquestion', 'items');
        ...
        if (!array_key_exists($gradeitemid, $cacheditems)) {
            return get_string('missing', 'availability_quizquestion');
        }
    ...
    protected static function get_cached_grade_score($gradeitemid, $courseid,
        ...
        $cache = \cache::make('availability_quizquestion', 'scores');
